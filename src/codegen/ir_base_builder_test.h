...
struct ProxyArgTrait<udf::Nullable<T>> {
    static void expand(udf::Nullable<T>* ptr, std::vector<int8_t*>* proxy_args) {
        if (ptr->is_null()) {
...
struct ProxyArgTrait<udf::Tuple<T...>> {
    static void expand(udf::Tuple<T...>* ptr, std::vector<int8_t*>* proxy_args) { Impl<0, T...>()(ptr, proxy_args); }
    ...
    struct Impl<I, Head, Tail...> {
        void operator()(udf::Tuple<T...>* ptr, std::vector<int8_t*>* proxy_args) {
...
struct ProxyReturnTrait<udf::Nullable<T>> {
    static void expand(udf::Nullable<T>* addr, std::vector<int8_t*>* proxy_args) {
...
struct ProxyReturnTrait<udf::Tuple<T...>> {
    ...
    struct Impl<I, Head, Tail...> {
        void operator()(udf::Tuple<T...>* ptr, std::vector<int8_t*>* proxy_args) {
            ProxyReturnTrait<Head>::expand(&(std::get<I>(ptr->tuple)), proxy_args);
            Impl<I + 1, Tail...>()(ptr, proxy_args);
        }
    };
...
struct FillProxyArgs<Head, Tail...> {
    void operator()(Head* head, Tail*... tail, std::vector<int8_t*>* proxy_args) {
...
struct FillProxyArgs<Head> {
    void operator()(Head* head, std::vector<int8_t*>* proxy_args) { ProxyArgTrait<Head>::expand(head, proxy_args); }
};

template <typename Ret>
struct FillProxyReturns {
    void operator()(Ret* addr, std::vector<int8_t*>* proxy_args) { ProxyReturnTrait<Ret>::expand(addr, proxy_args); }
};
...
class ModuleTestFunction {
 public:
    ...
    ModuleTestFunction(ModuleTestFunction&& inst) : jit(std::move(inst.jit)), fn_ptr(inst.fn_ptr) {}
 ...
 private:
    ...
    ModuleTestFunction(const std::string& fn_name, const std::string& proxy_fn_name, udf::UdfLibrary* library,
                       std::unique_ptr<::llvm::Module> module, std::unique_ptr<::llvm::LLVMContext> llvm_ctx) {
        llvm::InitializeNativeTarget();
        llvm::InitializeNativeTargetAsmPrinter();
        jit = std::unique_ptr<vm::HybridSeJitWrapper>(vm::HybridSeJitWrapper::Create());
        jit->Init();
        ...
        this->proxy_fn_ptr = const_cast<int8_t*>(jit->FindFunction(proxy_fn_name));
    }
...
class ModuleFunctionBuilderWithFullInfo {
 public:
    explicit ModuleFunctionBuilderWithFullInfo(BuilderStatePtr&& state) : state(std::move(state)) {}
    ...
    ModuleTestFunction<Ret, Args...> build(const std::function<base::Status(CodeGenContext*)>&);

 private:
    static size_t GetProxyArgCount(const node::TypeNode* arg_type);

    static void ExpandApplyArg(const node::TypeNode* arg_type, bool nullable, ::llvm::Function* function,
                               size_t* arg_idx, std::vector<::llvm::Value*>* arg_vec);

    static void ExpandApplyReturn(const node::TypeNode* ret_type, bool nullable, ::llvm::Function* function,
                                  size_t* arg_idx, std::vector<::llvm::Value*>* arg_vec,
                                  std::vector<int>* arg_nullable);
...
class ModuleFunctionBuilderWithArgs {
 public:
    explicit ModuleFunctionBuilderWithArgs(BuilderStatePtr&& state) : state(std::move(state)) {}
    ...
    auto returns() {
        ...
        return ModuleFunctionBuilderWithFullInfo<Ret, Args...>(std::move(state));
    }
...
class ModuleFunctionBuilderWithRet {
 public:
    explicit ModuleFunctionBuilderWithRet(BuilderStatePtr&& state) : state(std::move(state)) {}
...
template <typename Ret, typename... Args>
void ModuleFunctionBuilderWithFullInfo<Ret, Args...>::ExpandApplyArg(const node::TypeNode* arg_type, bool nullable,
    ...
    if (arg_type->base() == node::kTuple) {
        for (size_t i = 0; i < arg_type->GetGenericSize(); ++i) {
            ExpandApplyArg(arg_type->GetGenericType(i), arg_type->IsGenericNullable(i), function, arg_idx, arg_vec);
        }
    } else {
        ...
        ::llvm::Value* arg = builder.CreateLoad(builder.CreateGEP(arg_arr, builder.getInt64(*arg_idx)));
        *arg_idx += 1;
        if (nullable) {
            ...
            if (TypeIRBuilder::IsStructPtr(expect_ty)) {
                ...
                struct_builder->CreateDefault(&function->getEntryBlock(), &alloca);
                arg = builder.CreateSelect(is_null, alloca, builder.CreatePointerCast(arg, expect_ty));
            } else {
...
template <typename Ret, typename... Args>
void ModuleFunctionBuilderWithFullInfo<Ret, Args...>::ExpandApplyReturn(const node::TypeNode* ret_type, bool nullable,
                                                                        ::llvm::Function* function, size_t* arg_idx,
                                                                        std::vector<::llvm::Value*>* arg_vec,
                                                                        std::vector<int>* arg_nullable) {
    if (ret_type->base() == node::kTuple) {
        for (size_t i = 0; i < ret_type->GetGenericSize(); ++i) {
            ExpandApplyReturn(ret_type->GetGenericType(i), ret_type->IsGenericNullable(i), function, arg_idx, arg_vec,
                              arg_nullable);
        }
    } else {
        ...
        *arg_idx += 1;
        ...
        arg = builder.CreatePointerCast(arg, reinterpret_cast<::llvm::PointerType*>(expect_ty));  // T*
        arg_vec->push_back(arg);

        if (nullable) {
            ::llvm::Value* is_null_addr = builder.CreateLoad(builder.CreateGEP(arg_arr, builder.getInt64(*arg_idx)));
            *arg_idx += 1;
            is_null_addr =
                builder.CreatePointerCast(is_null_addr, ::llvm::Type::getInt1Ty(builder.getContext())->getPointerTo());
...
template <typename Ret, typename... Args>
ModuleTestFunction<Ret, Args...> ModuleFunctionBuilderWithFullInfo<Ret, Args...>::build(
    const std::function<base::Status(CodeGenContext*)>& build_module) {
    ModuleTestFunction<Ret, Args...> nil;
    auto llvm_ctx = std::unique_ptr<::llvm::LLVMContext>(new llvm::LLVMContext());
    auto module = std::unique_ptr<::llvm::Module>(new ::llvm::Module("Test", *llvm_ctx));
    ...
    auto status = GetLlvmFunctionType(module.get(), arg_types, arg_nullable, ret_type, ret_nullable, false,
                                      &return_by_arg, &function_ty);
    if (!status.isOK()) {
        LOG(WARNING) << status;
        return nil;
    }
    ...
    auto function = ::llvm::Function::Create(function_ty, llvm::Function::ExternalLinkage, fn_name, module.get());
    ...
    auto proxy_function_ty =
        ::llvm::FunctionType::get(::llvm::Type::getVoidTy(*llvm_ctx.get()), {ptr_ty->getPointerTo()}, false);
    auto proxy_function =
        ::llvm::Function::Create(proxy_function_ty, llvm::Function::ExternalLinkage, proxy_fn_name, module.get());
    auto proxy_block = ::llvm::BasicBlock::Create(*llvm_ctx, "entry", proxy_function);
    ::llvm::IRBuilder<> builder(proxy_block);
    std::vector<::llvm::Value*> llvm_apply_args;
    size_t llvm_arg_idx = 0;
    for (size_t i = 0; i < arg_types.size(); ++i) {
        ExpandApplyArg(arg_types[i], arg_nullable[i], proxy_function, &llvm_arg_idx, &llvm_apply_args);
    }
    std::vector<int> ret_has_null_flag;
    ExpandApplyReturn(ret_type, ret_nullable, proxy_function, &llvm_arg_idx, &llvm_apply_args, &ret_has_null_flag);
    auto callee = module->getOrInsertFunction(fn_name, function_ty);
    builder.CreateCall(callee, llvm_apply_args);
    builder.CreateRetVoid();

    return ModuleTestFunction<Ret, Args...>(fn_name, proxy_fn_name, state->library, std::move(module),
                                            std::move(llvm_ctx));
}

static inline NativeValue BindValueFromLlvmFunction(const node::TypeNode* arg_type, bool nullable,
                                                    ::llvm::Function* llvm_func, size_t* arg_idx) {
    if (arg_type->base() == node::kTuple) {
        std::vector<NativeValue> sub_vec;
        for (size_t i = 0; i < arg_type->GetGenericSize(); ++i) {
            NativeValue sub_field = BindValueFromLlvmFunction(arg_type->GetGenericType(i),
                                                              arg_type->IsGenericNullable(i), llvm_func, arg_idx);
...
static void WriteReturnValueArgs(const NativeValue& value, const node::TypeNode* ret_type, bool nullable,
                                 CodeGenContext* ctx, ::llvm::Function* llvm_func, size_t* arg_idx) {
    auto builder = ctx->GetBuilder();
    if (ret_type->base() == node::kTuple) {
        for (size_t i = 0; i < ret_type->GetGenericSize(); ++i) {
            WriteReturnValueArgs(value.GetField(i), ret_type->GetGenericType(i), ret_type->IsGenericNullable(i), ctx,
                                 llvm_func, arg_idx);
        }
    } else if (nullable) {
...
static node::ExprNode* ApplyExprFuncHelper(node::NodeManager* nm, const std::vector<node::ExprIdNode*>& args,
...
ModuleTestFunction<Ret, Args...> BuildExprFunction(
    ...
    const std::function<node::ExprNode*(node::NodeManager*, typename std::pair<Args, node::ExprNode*>::second_type...)>&
        expr_func) {
    return ModuleFunctionBuilder().library(library).returns<Ret>().template args<Args...>().build(
        [&](CodeGenContext* ctx) {
            node::NodeManager& nm = *ctx->node_manager();

            std::vector<node::TypeNode*> arg_types = {udf::DataTypeTrait<Args>::to_type_node(&nm)...};
            std::vector<int> arg_nullable = {udf::IsNullableTrait<Args>::value...};
            ...
            node::ExprNode* body = ApplyExprFuncHelper(&nm, arg_exprs, std::index_sequence_for<Args...>(), expr_func);
            CHECK_TRUE(body != nullptr, kCodegenError, "Build output expr failed");

            // type infer
            vm::SchemasContext empty_context;
            node::ExprAnalysisContext expr_pass_ctx(&nm, library, &empty_context);
            ...
            size_t llvm_arg_idx = 0;
            for (size_t i = 0; i < sizeof...(Args); ++i) {
                auto value = BindValueFromLlvmFunction(arg_types[i], arg_nullable[i], llvm_func, &llvm_arg_idx);
            ...
            WriteReturnValueArgs(out, ret_type, ret_nullable, ctx, llvm_func, &llvm_arg_idx);
...
ModuleTestFunction<Ret, Args...> BuildExprFunction(
    const std::function<node::ExprNode*(node::NodeManager*, typename std::pair<Args, node::ExprNode*>::second_type...)>&
        expr_func) {
    return BuildExprFunction<Ret, Args...>(udf::DefaultUdfLibrary::get(), expr_func);
}
