...
MemoryWindowDecodeIRBuilder::MemoryWindowDecodeIRBuilder(const vm::SchemasContext* schemas_context,
                                                         ::llvm::BasicBlock* block)
    : block_(block), schemas_context_(schemas_context) {}

MemoryWindowDecodeIRBuilder::~MemoryWindowDecodeIRBuilder() {}

bool MemoryWindowDecodeIRBuilder::BuildInnerRowsList(::llvm::Value* list_ptr, int64_t start_offset, int64_t end_offset,
                                                     ::llvm::Value** output) {
    ...
    uint32_t inner_list_size = sizeof(::hybridse::codec::InnerRowsList<hybridse::codec::Row>);
    ...
    ::llvm::FunctionCallee callee = block_->getModule()->getOrInsertFunction(
        "hybridse_storage_get_inner_rows_list", i32_ty, i8_ptr_ty, i64_ty, i64_ty, i8_ptr_ty);
    builder.CreateCall(callee,
                       ::llvm::ArrayRef<::llvm::Value*>{list_ptr, val_start_offset, val_end_offset, inner_list_ptr});
    *output = inner_list_ptr;
    return true;
}
bool MemoryWindowDecodeIRBuilder::BuildInnerRangeList(::llvm::Value* list_ptr, ::llvm::Value* row_key,
    ...
    ::llvm::Type* i32_ty = builder.getInt32Ty();
    ::llvm::Type* i64_ty = builder.getInt64Ty();
    uint32_t inner_list_size = sizeof(::hybridse::codec::InnerRangeList<hybridse::codec::Row>);
    // alloca memory on stack for col iterator
    ::llvm::ArrayType* array_type = ::llvm::ArrayType::get(i8_ty, inner_list_size);
    ::llvm::Value* inner_list_ptr = CreateAllocaAtHead(&builder, array_type, "sub_window_alloca");
    inner_list_ptr = builder.CreatePointerCast(inner_list_ptr, i8_ptr_ty);
    ...
    ::llvm::Value* val_end_offset = builder.getInt64(end_offset);
    ::llvm::FunctionCallee callee = block_->getModule()->getOrInsertFunction(
        "hybridse_storage_get_inner_range_list", i32_ty, i8_ptr_ty, i64_ty, i64_ty, i64_ty, i8_ptr_ty);
    builder.CreateCall(
...
bool MemoryWindowDecodeIRBuilder::BuildGetCol(size_t schema_idx, size_t col_idx, ::llvm::Value* window_ptr,
                                              ::llvm::Value** output) {
    ...
    if (col_info == nullptr) {
        LOG(WARNING) << "fail to get column info at " << schema_idx << ":" << col_idx;
        return false;
    }
    ...
    switch (data_type.base_) {
        ...
        case ::hybridse::node::kDate: {
            return BuildGetPrimaryCol("hybridse_storage_get_col", window_ptr, schema_idx, col_idx, col_info->offset,
                                      &data_type, output);
        }
        case ::hybridse::node::kVarchar: {
            codec::StringColInfo str_col_info;
            if (!schemas_context_->GetRowFormat(schema_idx)->GetStringColumnInfo(col_idx, &str_col_info)) {
                LOG(WARNING) << "fail to get string filed offset and next offset"
                             << " at " << col_idx;
            }
            DLOG(INFO) << "get string with offset " << str_col_info.offset << " next offset "
                       << str_col_info.str_next_offset << " for col at " << str_col_info.name;
        ...
        default: {
            LOG(WARNING) << "Fail get col, invalid data type " << data_type.GetName();
            return false;
        }
    }
}  // namespace codegen

bool MemoryWindowDecodeIRBuilder::BuildGetPrimaryCol(const std::string& fn_name, ::llvm::Value* row_ptr,
                                                     size_t schema_idx, size_t col_idx, uint32_t offset,
                                                     hybridse::node::TypeNode* type, ::llvm::Value** output) {
    ...
    // alloca memory on stack for col iterator
    ::llvm::ArrayType* array_type = ::llvm::ArrayType::get(i8_ty, col_iterator_size);
    ::llvm::Value* col_iter = CreateAllocaAtHead(&builder, array_type, "col_iter_alloca");
    // alloca memory on stack
    ::llvm::Value* list_ref = CreateAllocaAtHead(&builder, list_ref_type, "list_ref_alloca");
    ::llvm::Value* data_ptr_ptr = builder.CreateStructGEP(list_ref_type, list_ref, 0);
    ...
    if (!DataType2SchemaType(*type, &schema_type)) {
        LOG(WARNING) << "fail to convert data type to schema type: " << type->GetName();
        return false;
    }

    ::llvm::Value* val_type_id = builder.getInt32(static_cast<int32_t>(schema_type));
    ::llvm::FunctionCallee callee =
        block_->getModule()->getOrInsertFunction(fn_name, i32_ty, i8_ptr_ty, i32_ty, i32_ty, i32_ty, i32_ty, i8_ptr_ty);
    builder.CreateCall(callee, {row_ptr, val_schema_idx, val_col_idx, val_offset, val_type_id, col_iter});
    *output = list_ref;
    return true;
}

bool MemoryWindowDecodeIRBuilder::BuildGetStringCol(size_t schema_idx, size_t col_idx, uint32_t offset,
    ...
    ::llvm::FunctionCallee callee = block_->getModule()->getOrInsertFunction(
        "hybridse_storage_get_str_col", i32_ty, i8_ptr_ty, i32_ty, i32_ty, i32_ty, i32_ty, i32_ty, i32_ty, i8_ptr_ty);
    ...
    ::llvm::Value* val_type_id = builder.getInt32(static_cast<int32_t>(schema_type));
    builder.CreateCall(callee, {window_ptr, val_schema_idx, val_col_idx, str_offset, next_str_offset,
