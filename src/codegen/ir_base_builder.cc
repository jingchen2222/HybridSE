...
bool GetLlvmType(::llvm::Module* m, const ::hybridse::node::DataType& type, ::llvm::Type** llvm_type) {
    if (nullptr == m) {
    ...
    switch (type) {
        ...
        case node::kMap:
        case node::kIterator: {
            LOG(WARNING) << "fail to convert type" << node::DataTypeName(type) << "without generic types";
            return false;
        }
        case node::kOpaque:
        case node::kRow: {
...
bool GetLlvmListType(::llvm::Module* m, const ::hybridse::node::TypeNode* v_type, ::llvm::Type** output) {
    if (output == NULL) {
        LOG(WARNING) << "the output ptr is NULL ";
        return false;
    }
    ...
    switch (v_type->base_) {
        ...
        default: {
            LOG(WARNING) << "not supported list<type> when type is  " << v_type->GetName();
            return false;
        }
    }
...
bool GetLlvmIteratorType(::llvm::Module* m, const ::hybridse::node::TypeNode* v_type, ::llvm::Type** output) {
    if (output == NULL) {
        LOG(WARNING) << "the output ptr is NULL ";
        return false;
    }
...
bool GetLlvmType(::llvm::Module* m, const hybridse::node::TypeNode* data_type, ::llvm::Type** llvm_type) {
    ...
    switch (data_type->base_) {
        case hybridse::node::kList: {
            ...
            if (false == GetLlvmListType(m, data_type->generics_[0], &list_type)) {
                return false;
            }
        ...
        case hybridse::node::kIterator: {
            if (data_type->generics_.size() != 1) {
                LOG(WARNING) << "fail to convert data type: iterator generic types "
                                "number is "
                             << data_type->generics_.size();
                return false;
            }
            ::llvm::Type* list_type = nullptr;
            if (false == GetLlvmIteratorType(m, data_type->generics_[0], &list_type)) {
                return false;
            }
            *llvm_type = list_type->getPointerTo();
            return true;
...
bool GetFullType(node::NodeManager* nm, ::llvm::Type* type, const ::hybridse::node::TypeNode** type_node) {
    ...
    if (false == GetBaseType(type, &base)) {
        LOG(WARNING) << "Fail to get base type of " << GetLlvmObjectString(type);
        return false;
    }
    ...
    switch (base) {
        case hybridse::node::kList: {
            ...
            if (type->getStructName().equals("fe.list_ref_int16")) {
                *type_node = nm->MakeTypeNode(base, nm->MakeTypeNode(hybridse::node::kInt16));
                return true;
            } else if (type->getStructName().equals("fe.list_ref_int32")) {
                *type_node = nm->MakeTypeNode(base, nm->MakeTypeNode(hybridse::node::kInt32));
                return true;
            } else if (type->getStructName().equals("fe.list_ref_int64")) {
                *type_node = nm->MakeTypeNode(base, nm->MakeTypeNode(hybridse::node::kInt64));
                return true;
            } else if (type->getStructName().equals("fe.list_ref_bool")) {
                *type_node = nm->MakeTypeNode(base, nm->MakeTypeNode(hybridse::node::kBool));
                return true;
            } else if (type->getStructName().equals("fe.list_ref_float")) {
                *type_node = nm->MakeTypeNode(base, nm->MakeTypeNode(hybridse::node::kFloat));
                return true;
            } else if (type->getStructName().equals("fe.list_ref_double")) {
                *type_node = nm->MakeTypeNode(base, nm->MakeTypeNode(hybridse::node::kDouble));
                return true;
            } else if (type->getStructName().equals("fe.list_ref_string")) {
                *type_node = nm->MakeTypeNode(base, nm->MakeTypeNode(hybridse::node::kVarchar));
                return true;
            } else if (type->getStructName().equals("fe.list_ref_timestamp")) {
                *type_node = nm->MakeTypeNode(base, nm->MakeTypeNode(hybridse::node::kTimestamp));
                return true;
            } else if (type->getStructName().equals("fe.list_ref_date")) {
                *type_node = nm->MakeTypeNode(base, nm->MakeTypeNode(hybridse::node::kDate));
                return true;
            }
            LOG(WARNING) << "fail to get type of llvm type for " << type->getStructName().str();
            return false;
        }
        case hybridse::node::kIterator: {
            if (type->getTypeID() == ::llvm::Type::PointerTyID) {
            ...
            if (type->getStructName().equals("fe.iterator_ref_int16")) {
                ...
                *type_node = nm->MakeTypeNode(base, nm->MakeTypeNode(hybridse::node::kVarchar));
                return true;
            } else if (type->getStructName().equals("fe.iterator_ref_timestamp")) {
                *type_node = nm->MakeTypeNode(base, nm->MakeTypeNode(hybridse::node::kTimestamp));
                return true;
...
bool GetBaseType(::llvm::Type* type, ::hybridse::node::DataType* output) {
    ...
    switch (type->getTypeID()) {
        ...
        case ::llvm::Type::PointerTyID: {
            ...
            LOG(WARNING) << "no mapping type for llvm type " << type->getStructName().str();
            return false;
        }
        default: {
            LOG(WARNING) << "no mapping type for llvm type: " << GetLlvmObjectString(type);
            return false;
        }
    }
}
...
bool BuildStoreOffset(::llvm::IRBuilder<>& builder,  // NOLINT
    ...
    bool ok = BuildGetPtrOffset(builder, ptr, offset, value->getType()->getPointerTo(), &ptr_with_offset);
...
bool DataType2SchemaType(const ::hybridse::node::TypeNode& type, ::hybridse::type::Type* output) {
    switch (type.base_) {
        ...
        default: {
            LOG(WARNING) << "can't convert to schema for type: " << type.GetName();
            return false;
        }
    }
    return true;
...
bool TypeIRBuilder::IsInterger(::llvm::Type* type) { return type->isIntegerTy(); }
bool TypeIRBuilder::IsNumber(::llvm::Type* type) { return type->isIntegerTy() || type->isFloatingPointTy(); }
bool TypeIRBuilder::isFloatPoint(::llvm::Type* type) { return type->isFloatingPointTy(); }
...
static Status ExpandLlvmArgTypes(::llvm::Module* m, const node::TypeNode* dtype, bool nullable,
    ...
    if (dtype->base() == node::kTuple) {
        ...
        for (size_t i = 0; i < dtype->GetGenericSize(); ++i) {
            CHECK_STATUS(ExpandLlvmArgTypes(m, dtype->GetGenericType(i), dtype->IsGenericNullable(i), output),
                         kCodegenError);
        }
    } else {
...
static Status ExpandLlvmReturnTypes(::llvm::Module* m, const node::TypeNode* dtype, bool nullable,
                                    std::vector<std::pair<::llvm::Type*, bool>>* output) {
    if (dtype->base() == node::kTuple) {
        CHECK_TRUE(!nullable, kCodegenError, "kTuple should never be nullable");
        for (size_t i = 0; i < dtype->GetGenericSize(); ++i) {
            CHECK_STATUS(ExpandLlvmReturnTypes(m, dtype->GetGenericType(i), dtype->IsGenericNullable(i), output));
        }
    } else {
        ::llvm::Type* llvm_ty = nullptr;
        CHECK_TRUE(GetLlvmType(m, dtype, &llvm_ty), kCodegenError, "Fail to lower ", dtype->GetName());
...
Status GetLlvmFunctionType(::llvm::Module* m, const std::vector<const node::TypeNode*>& arg_types,
                           const std::vector<int>& arg_nullable, const node::TypeNode* return_type,
                           bool return_nullable, bool variadic, bool* return_by_arg, ::llvm::FunctionType** output) {
    // expand raw llvm arg types from signature
    std::vector<std::pair<::llvm::Type*, bool>> llvm_arg_types;
    for (size_t i = 0; i < arg_types.size(); ++i) {
        CHECK_STATUS(ExpandLlvmArgTypes(m, arg_types[i], arg_nullable[i], &llvm_arg_types))
    }
    ...
    CHECK_STATUS(ExpandLlvmReturnTypes(m, return_type, return_nullable, &llvm_ret_types))
    CHECK_TRUE(llvm_ret_types.size() > 0, kCodegenError);

    if (llvm_ret_types.size() > 1 || TypeIRBuilder::IsStructPtr(llvm_ret_types[0].first) || llvm_ret_types[0].second) {
    ...
    *output = llvm::FunctionType::get(ret_llvm_ty, expand_llvm_arg_types, variadic);
    return Status::OK();
}
