 ...
 * @return
 */
base::Status CondSelectIRBuilder::Select(::llvm::BasicBlock* block, const NativeValue& cond_value,
                                         const NativeValue& left, const NativeValue& right, NativeValue* output) {
    ...
    if (left_value.IsTuple()) {
        CHECK_TRUE(right_value.IsTuple() && left_value.GetFieldNum() == right_value.GetFieldNum(), kCodegenError);
        ...
        for (size_t i = 0; i < left_value.GetFieldNum(); ++i) {
            ...
            ::llvm::Value* raw_value =
                builder.CreateSelect(raw_cond, sub_left.GetValue(&builder), sub_right.GetValue(&builder));

            bool output_nullable = sub_left.IsNullable() || sub_right.IsNullable();
            if (output_nullable) {
                ::llvm::Value* output_is_null =
                    builder.CreateSelect(raw_cond, sub_left.GetIsNull(&builder), sub_right.GetIsNull(&builder));
        ...
        *output = NativeValue::CreateTuple(result_tuple);
        ...
        ::llvm::Value* raw_value =
            builder.CreateSelect(raw_cond, left_value.GetValue(&builder), right_value.GetValue(&builder));
        bool output_nullable = left_value.IsNullable() || right_value.IsNullable();
        if (output_nullable) {
            ::llvm::Value* output_is_null =
                builder.CreateSelect(raw_cond, left_value.GetIsNull(&builder), right_value.GetIsNull(&builder));
