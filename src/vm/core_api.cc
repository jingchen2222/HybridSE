...
bool WindowInterface::BufferData(uint64_t key, const Row& row) { return window_impl_->BufferData(key, row); }

Window::WindowFrameType WindowInterface::ExtractFrameType(const std::string& frame_type_str) const {
...
int CoreAPI::ResolveColumnIndex(hybridse::vm::PhysicalOpNode* node, hybridse::node::ExprNode* expr) {
    ...
    switch (expr->GetExprType()) {
        case node::kExprColumnRef: {
            ...
            status = schemas_ctx->ResolveColumnRefIndex(column_ref, &schema_idx, &col_idx);
            break;
        }
        case node::kExprColumnId: {
            auto column_id = dynamic_cast<const node::ColumnIdNode*>(expr);
            status = schemas_ctx->ResolveColumnIndexByID(column_id->GetColumnID(), &schema_idx, &col_idx);
            break;
        }
        default:
            return -1;
...
std::string CoreAPI::ResolveSourceColumnName(hybridse::vm::PhysicalOpNode* node, hybridse::node::ColumnRefNode* expr) {
    const SchemasContext* schemas_ctx = node->schemas_ctx();
    ...
    auto status = schemas_ctx->ResolveColumnID(column_expr->GetRelationName(), column_expr->GetColumnName(), &column_id,
                                               &child_path_idx, &child_column_id, &source_column_id, &source_node);
    if (!status.isOK() || source_node == nullptr) {
        LOG(WARNING) << "Fail to resolve column " << column_expr->GetExprString();
        return "";
    }
    size_t schema_idx;
    size_t col_idx;
    status = source_node->schemas_ctx()->ResolveColumnIndexByID(source_column_id, &schema_idx, &col_idx);
    return source_node->schemas_ctx()->GetSchemaSource(schema_idx)->GetColumnName(col_idx);
}

ColumnSourceInfo CoreAPI::ResolveSourceColumn(hybridse::vm::PhysicalOpNode* node, const std::string& relation_name,
    ...
    status =
        node->schemas_ctx()->ResolveColumnID(relation_name, column_name, &result.column_id_, &result.child_path_idx_,
                                             &result.child_column_id_, &result.source_column_id_, &result.source_node_);
    ...
    size_t schema_idx;
    size_t col_idx;
    status = node->schemas_ctx()->ResolveColumnIndexByID(result.column_id_, &schema_idx, &col_idx);
    if (!status.isOK()) {
        return result;
    }
    ...
    status = source_schemas_ctx->ResolveColumnIndexByID(result.source_column_id_, &source_schema_idx, &source_col_idx);
    if (!status.isOK()) {
        return result;
    }
    ...
    for (size_t i = 0; i < source_schema_idx; ++i) {
        result.source_total_col_idx_ += source_schemas_ctx->GetSchemaSource(i)->size();
    }
    result.source_col_name_ = source_schemas_ctx->GetSchemaSource(source_schema_idx)->GetColumnName(source_col_idx);
    return result;
}

size_t CoreAPI::GetUniqueID(const hybridse::vm::PhysicalOpNode* node) { return node->node_id(); }

GroupbyInterface::GroupbyInterface(const hybridse::codec::Schema& schema)
    : mem_table_handler_(new vm::MemTableHandler(&schema)) {}

void GroupbyInterface::AddRow(hybridse::codec::Row* row) { mem_table_handler_->AddRow(*row); }

hybridse::vm::TableHandler* GroupbyInterface::GetTableHandler() { return mem_table_handler_; }

hybridse::codec::Row CoreAPI::RowConstProject(const RawPtrHandle fn, const bool need_free) {
...
hybridse::codec::Row CoreAPI::RowProject(const RawPtrHandle fn, const hybridse::codec::Row row, const bool need_free) {
    if (row.empty()) {
        return hybridse::codec::Row();
    }
    // Init current run step runtime
...
void CoreAPI::CopyRowToUnsafeRowBytes(const hybridse::codec::Row inputRow, hybridse::vm::ByteArrayPtr outputBytes,
                                      const int length) {
    memcpy(outputBytes, inputRow.buf() + codec::HEADER_LENGTH, length);
}

hybridse::codec::Row CoreAPI::WindowProject(const RawPtrHandle fn, const uint64_t row_key, const Row row,
    ...
    uint32_t ret = udf(static_cast<int64_t>(row_key), row_ptr, window_ptr, &out_buf);
...
hybridse::codec::Row CoreAPI::WindowProject(const RawPtrHandle fn, const uint64_t key, const Row row,
                                            const bool is_instance, size_t append_slices, WindowInterface* window) {
    return Runner::WindowProject(fn, key, row, is_instance, append_slices, window->GetWindow());
}

hybridse::codec::Row CoreAPI::UnsafeWindowProject(const RawPtrHandle fn, const uint64_t key,
                                                  hybridse::vm::ByteArrayPtr inputUnsafeRowBytes,
...
hybridse::codec::Row CoreAPI::GroupbyProject(const RawPtrHandle fn, hybridse::vm::GroupbyInterface* groupby_interface) {
...
bool CoreAPI::ComputeCondition(const hybridse::vm::RawPtrHandle fn, const Row& row,
    ...
    return Runner::GetColumnBool(cond_row.buf(), row_view, out_idx, row_view->GetSchema()->Get(out_idx).type());
}
...
RawPtrHandle CoreAPI::GetRowBuf(hybridse::codec::Row* row, size_t idx) { return row->buf(idx); }
