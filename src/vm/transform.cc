            ...
            switch (join_op->join().join_type()) {
                ...
                case node::kJoinTypeLast: {
                    ...
                    if (nullptr != orders && !node::ExprListNullOrEmpty(orders->order_by_)) {
                        CHECK_STATUS(CheckExprDependOnChildOnly(orders->order_by_, child_schemas_ctx),
                                     "Fail to handle window: group "
                                     "expression should belong to left table");
                    }
...
Status BatchModeTransformer::ValidateJoinIndexOptimization(const Join& join, PhysicalOpNode* right) {
    ...
    if (node::kJoinTypeLast == join.join_type_) {
        CHECK_TRUE(
            nullptr == join.right_sort_.orders() || node::ExprListNullOrEmpty(join.right_sort_.orders()->order_by_),
...
Status BatchModeTransformer::ValidateWindowIndexOptimization(const WindowOp& window, PhysicalOpNode* in) {
    ...
    CHECK_TRUE(!window.sort().ValidSort() || node::ExprListNullOrEmpty(window.sort().orders()->order_by_), kPlanError,
    ...
    for (; it != trees.end(); ++it) {
        ...
        switch (node->GetType()) {
            ...
            case ::hybridse::node::kPlanTypeQuery: {
                ...
                CHECK_STATUS(ValidatePlan(optimized_physical_plan), "Fail to generate physical plan, invalid plan");
...
Status BatchModeTransformer::GenSort(Sort* sort, const SchemasContext* schemas_ctx) {
    if (nullptr != sort->orders_ && !node::ExprListNullOrEmpty(sort->orders()->order_by())) {
        return plan_ctx_.InitFnDef(sort->orders()->order_by(), schemas_ctx, true, sort);
    }
    return Status::OK();
}
...
Status BatchModeTransformer::CheckTimeOrIntegerOrderColumn(const node::OrderByNode* orders,
    ...
    if (nullptr != orders && !node::ExprListNullOrEmpty(orders->order_by_)) {
        if (1u != orders->order_by_->children_.size()) {
            return base::Status(common::kPlanError, "Invalid Order Column: can't support multi order");
        }

        auto order = orders->order_by_->children_[0];
        if (node::kExprColumnRef != order->expr_type_) {
            return base::Status(common::kPlanError, "Invalid Order Column: can't support expression order");
        }
        size_t schema_idx;
        size_t col_idx;
        CHECK_STATUS(
            schemas_ctx->ResolveColumnRefIndex(dynamic_cast<node::ColumnRefNode*>(order), &schema_idx, &col_idx));
...
Status BatchModeTransformer::CheckWindow(const node::WindowPlanNode* w_ptr, const vm::SchemasContext* schemas_ctx) {
    ...
    CHECK_TRUE(nullptr != w_ptr->GetOrders() && !node::ExprListNullOrEmpty(w_ptr->GetOrders()->order_by_),
