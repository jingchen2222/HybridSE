...
namespace hybridse {
namespace vm {
MemTimeTableIterator::MemTimeTableIterator(const MemTimeTable* table, const vm::Schema* schema)
    : table_(table), schema_(schema), start_iter_(table->cbegin()), end_iter_(table->cend()), iter_(table->cbegin()) {}
MemTimeTableIterator::MemTimeTableIterator(const MemTimeTable* table, const vm::Schema* schema, int32_t start,
                                           int32_t end)
    : table_(table),
      schema_(schema),
...
MemWindowIterator::MemWindowIterator(const MemSegmentMap* partitions, const Schema* schema)
    : WindowIterator(),
...
void MemWindowIterator::Seek(const std::string& key) { iter_ = partitions_->find(key); }
...
std::unique_ptr<RowIterator> MemWindowIterator::GetValue() {
    return std::unique_ptr<RowIterator>(new MemTimeTableIterator(&(iter_->second), schema_));
}
...
MemTimeTableHandler::MemTimeTableHandler(const Schema* schema)
    : TableHandler(),
      ...
      table_(),
      order_type_(kNoneOrder) {}
MemTimeTableHandler::MemTimeTableHandler(const std::string& table_name, const std::string& db, const Schema* schema)
    : TableHandler(),
...
std::unique_ptr<RowIterator> MemTimeTableHandler::GetIterator() {
    std::unique_ptr<MemTimeTableIterator> it(new MemTimeTableIterator(&table_, schema_));
    return std::move(it);
}
std::unique_ptr<WindowIterator> MemTimeTableHandler::GetWindowIterator(const std::string& idx_name) {
    return std::unique_ptr<WindowIterator>();
}

void MemTimeTableHandler::AddRow(const uint64_t key, const Row& row) { table_.emplace_back(std::make_pair(key, row)); }
...
void MemTimeTableHandler::Reverse() {
    ...
    order_type_ = kAscOrder == order_type_ ? kDescOrder : kDescOrder == order_type_ ? kAscOrder : kNoneOrder;
}
RowIterator* MemTimeTableHandler::GetRawIterator() { return new MemTimeTableIterator(&table_, schema_); }

MemPartitionHandler::MemPartitionHandler()
    : PartitionHandler(), table_name_(""), db_(""), schema_(nullptr), order_type_(kNoneOrder) {}

MemPartitionHandler::MemPartitionHandler(const Schema* schema)
    : PartitionHandler(), table_name_(""), db_(""), schema_(schema), order_type_(kNoneOrder) {}
MemPartitionHandler::MemPartitionHandler(const std::string& table_name, const std::string& db, const Schema* schema)
    : PartitionHandler(), table_name_(table_name), db_(db), schema_(schema), order_type_(kNoneOrder) {}
MemPartitionHandler::~MemPartitionHandler() {}
...
bool MemPartitionHandler::AddRow(const std::string& key, uint64_t ts, const Row& row) {
    auto iter = partitions_.find(key);
    if (iter == partitions_.cend()) {
        partitions_.insert(std::pair<std::string, MemTimeTable>(key, {std::make_pair(ts, row)}));
    } else {
...
std::unique_ptr<WindowIterator> MemPartitionHandler::GetWindowIterator() {
    return std::unique_ptr<WindowIterator>(new MemWindowIterator(&partitions_, schema_));
}
...
std::unique_ptr<WindowIterator> MemTableHandler::GetWindowIterator(const std::string& idx_name) {
    return std::unique_ptr<WindowIterator>();
}
std::unique_ptr<RowIterator> MemTableHandler::GetIterator() {
    std::unique_ptr<MemTableIterator> it(new MemTableIterator(&table_, schema_));
    return std::move(it);
}
RowIterator* MemTableHandler::GetRawIterator() { return new MemTableIterator(&table_, schema_); }
...
MemTableHandler::MemTableHandler(const Schema* schema)
    : TableHandler(),
      ...
      table_(),
      order_type_(kNoneOrder) {}
MemTableHandler::MemTableHandler(const std::string& table_name, const std::string& db, const Schema* schema)
    : TableHandler(),
      table_name_(table_name),
      db_(db),
      schema_(schema),
...
MemTableHandler::~MemTableHandler() {}
MemTableIterator::MemTableIterator(const MemTable* table, const vm::Schema* schema)
    : table_(table),
      schema_(schema),
      ...
      iter_(table->cbegin()),
      key_(0) {}
MemTableIterator::MemTableIterator(const MemTable* table, const vm::Schema* schema, int32_t start, int32_t end)
    : table_(table),
      schema_(schema),
      start_iter_(table_->begin() + start),
      end_iter_(table_->begin() + end),
...
class RequestUnionIterator : public RowIterator {
 public:
    RequestUnionIterator(uint64_t request_ts, const Row* request_row, RowIterator* window_iter)
        : request_ts_(request_ts), request_row_(request_row), window_iter_(window_iter) {}
    ~RequestUnionIterator() { delete window_iter_; }
    bool Valid() const override { return window_iter_start_ ? window_iter_->Valid() : true; }
    void Next() override {
    ...
    const uint64_t& GetKey() const override { return window_iter_start_ ? window_iter_->GetKey() : request_ts_; }
    const Row& GetValue() override { return window_iter_start_ ? window_iter_->GetValue() : *request_row_; }
...
void GetRowIter(int8_t* input, int8_t* iter_addr) {
    ...
    auto local_iter = new (iter_addr) std::unique_ptr<RowIterator>(handler->GetIterator());
    (*local_iter)->SeekToFirst();
}
bool RowIterHasNext(int8_t* iter_ptr) {
    auto& local_iter = *reinterpret_cast<std::unique_ptr<RowIterator>*>(iter_ptr);
