...
CommonColumnOptimize::CommonColumnOptimize(const std::set<size_t> common_column_indices)
    : common_column_indices_(common_column_indices) {}

bool CommonColumnOptimize::FindRequestUnionPath(PhysicalOpNode* root, std::vector<PhysicalOpNode*>* path) {
    if (root->GetOutputType() == PhysicalSchemaType::kSchemaTypeRow) {
        return false;
    }
    path->push_back(root);
    if (root->GetOpType() == PhysicalOpType::kPhysicalOpRequestUnion) {
        return dynamic_cast<PhysicalRequestUnionNode*>(root)->output_request_row();
    }
...
static Status GetSingleSliceResult(PhysicalPlanContext* ctx, PhysicalOpNode* input, PhysicalOpNode** output) {
    if (input == nullptr) {
        *output = nullptr;
        return Status::OK();
    }
    ...
    for (size_t i = 0; i < input->GetOutputSchemaSourceSize(); ++i) {
        ...
        for (size_t j = 0; j < source->size(); ++j) {
            auto column = ctx->node_manager()->MakeColumnIdNode(source->GetColumnID(j));
            projects.Add(source->GetColumnName(j), column, nullptr);
        }
    }
    PhysicalSimpleProjectNode* op = nullptr;
    CHECK_STATUS(ctx->CreateOp<PhysicalSimpleProjectNode>(&op, input, projects));
    *output = op;
    return Status::OK();
}

Status CommonColumnOptimize::Apply(PhysicalPlanContext* ctx, PhysicalOpNode* input, PhysicalOpNode** output) {
    ...
    CHECK_STATUS(GetSingleSliceResult(ctx, state->non_common_op, &non_common_output));
    ...
    if (common_output == nullptr) {
        ...
        *output = common_output;
        ...
        CHECK_STATUS(
            ctx->CreateOp<PhysicalRequestJoinNode>(&concat, common_output, non_common_output, node::kJoinTypeConcat));
        *output = concat;
    }
    return Status::OK();
}

Status CommonColumnOptimize::ProcessRequest(PhysicalPlanContext* ctx, PhysicalRequestProviderNode* data_op,
                                            BuildOpState* state) {
    const codec::Schema* request_schema = data_op->GetOutputSchema();
    if (common_column_indices_.empty()) {
        state->SetAllNonCommon(data_op);
        return Status::OK();
    } else if (common_column_indices_.size() == static_cast<size_t>(request_schema->size())) {
    ...
    CHECK_STATUS(ctx->CreateOp<PhysicalRequestProviderNodeWithCommonColumn>(&new_data_op, data_op->table_handler_,
                                                                            common_column_indices_));
    ...
    CHECK_STATUS(ctx->CreateOp<PhysicalSimpleProjectNode>(&common_select_op, new_data_op, common_projects));
    ...
    CHECK_STATUS(ctx->CreateOp<PhysicalSimpleProjectNode>(&non_common_select_op, new_data_op, non_common_projects));
    state->non_common_op = non_common_select_op;
    return Status::OK();
}

Status CommonColumnOptimize::ProcessData(PhysicalPlanContext* ctx, PhysicalDataProviderNode* data_op,
                                         BuildOpState* state) {
    if (data_op->provider_type_ == DataProviderType::kProviderTypeRequest) {
        return ProcessRequest(ctx, dynamic_cast<PhysicalRequestProviderNode*>(data_op), state);
    } else {
        state->SetAllCommon(data_op);
        return Status::OK();
    }
 ...
 */
static bool ExprDependOnlyOnLeft(const node::ExprNode* expr, PhysicalOpNode* left, PhysicalOpNode* right) {
    if (left == nullptr) {
        return false;
    }
    std::set<size_t> columns;
    Status status = left->schemas_ctx()->ResolveExprDependentColumns(expr, &columns);
    if (!status.isOK()) {
        return false;
    }
    if (right == nullptr) {
...
static bool LeftDependOnlyOnPart(const Component& comp, PhysicalOpNode* part, PhysicalOpNode* origin_left,
    ...
    for (auto col : related_columns) {
        ...
        if (!part->schemas_ctx()->ResolveExprDependentColumns(col, &column_ids).isOK()) {
            return false;
        }
    }
    return true;
...
static Status UpdateProjectExpr(PhysicalPlanContext* ctx, const node::ExprNode* expr,
                                ...
                                const std::unordered_map<size_t, size_t>& new_id_map, node::ExprNode** output) {
    auto schemas_ctx = origin_input->schemas_ctx();
    std::vector<const node::ExprNode*> origin_columns;
    CHECK_STATUS(schemas_ctx->ResolveExprDependentColumns(expr, &origin_columns));
    ...
    for (auto column_expr : origin_columns) {
        ...
        if (column_expr->GetExprType() == node::kExprColumnId) {
            ...
            CHECK_STATUS(schemas_ctx->ResolveColumnIndexByID(column_id->GetColumnID(), &schema_idx, &col_idx));
...
static Status CreateSimplifiedProject(PhysicalPlanContext* ctx, PhysicalOpNode* input, const ColumnProjects& projects,
    ...
    while (can_project) {
        ...
        for (size_t i = 0; i < cur_input->producers().size(); ++i) {
            ...
            for (size_t j = 0; j < projects.size(); ++j) {
                if (!ExprDependOnlyOnLeft(projects.GetExpr(j), cand_input, nullptr)) {
                    is_valid = false;
                    break;
                }
            }
    ...
    CHECK_STATUS(ctx->CreateOp<PhysicalSimpleProjectNode>(&project_op, cur_input, projects));
    *output = project_op;
    return Status::OK();
}

Status CommonColumnOptimize::ProcessSimpleProject(PhysicalPlanContext* ctx, PhysicalSimpleProjectNode* project_op,
                                                  BuildOpState* state) {
    // fetch input states
    ...
    for (size_t i = 0; i < origin_projects.size(); ++i) {
        ...
        if (need_update) {
            ...
            CHECK_STATUS(UpdateProjectExpr(ctx, expr, input_op, new_id_map, &new_expr), "Fail to resolve expr ",
                         expr->GetExprString(), " on project input:\n", new_input->SchemaToString());
            expr = new_expr;
        }

        if (ExprDependOnlyOnLeft(expr, input_state->common_op, input_state->non_common_op)) {
            state->AddCommonIdx(i);
            common_projects.Add(name, expr, frame);
        } else if (ExprDependOnlyOnLeft(expr, input_state->non_common_op, input_state->common_op)) {
    ...
    if (common_projects.size() == origin_projects.size() && input_op == input_state->common_op) {
        state->common_op = project_op;
    } else if (common_projects.size() > 0) {
        CHECK_STATUS(CreateSimplifiedProject(ctx, input_state->common_op, common_projects, &state->common_op))
    } else {
        state->common_op = nullptr;
    }

    if (non_common_projects.size() == origin_projects.size() && input_op == new_input) {
        state->non_common_op = project_op;
    } else if (non_common_projects.size() > 0) {
        CHECK_STATUS(CreateSimplifiedProject(ctx, new_input, non_common_projects, &state->non_common_op))
    } else {
...
static Status CreateNewProject(PhysicalPlanContext* ctx, ProjectType ptype, PhysicalOpNode* input,
                               const ColumnProjects& projects, PhysicalOpNode** out) {
    switch (ptype) {
        case ProjectType::kRowProject: {
            PhysicalRowProjectNode* op = nullptr;
            CHECK_STATUS(ctx->CreateOp<PhysicalRowProjectNode>(&op, input, projects));
            *out = op;
            break;
        }
        case ProjectType::kTableProject: {
            PhysicalTableProjectNode* op = nullptr;
            CHECK_STATUS(ctx->CreateOp<PhysicalTableProjectNode>(&op, input, projects));
            *out = op;
            break;
        }
        case ProjectType::kAggregation: {
            PhysicalAggrerationNode* op = nullptr;
            CHECK_STATUS(ctx->CreateOp<PhysicalAggrerationNode>(&op, input, projects));
            *out = op;
            break;
        }
        default:
            return Status(kPlanError, "Unknown project type: " + ProjectTypeName(ptype));
    }
    return Status::OK();
}

Status CommonColumnOptimize::ProcessProject(PhysicalPlanContext* ctx, PhysicalProjectNode* project_op,
                                            BuildOpState* state) {
    // process window agg
    ...
    if (common_projects.size() == origin_projects.size() && input_state->common_op == input_op) {
        state->common_op = project_op;
    } else if (common_projects.size() > 0) {
        PhysicalOpNode* common_project_op = nullptr;
        CHECK_STATUS(CreateNewProject(ctx, project_op->project_type_, input_state->common_op, common_projects,
                                      &common_project_op));
    ...
    if (non_common_projects.size() == origin_projects.size() && new_input == input_op) {
        state->non_common_op = project_op;
    } else if (non_common_projects.size() > 0) {
        PhysicalOpNode* non_common_project_op = nullptr;
        CHECK_STATUS(
            CreateNewProject(ctx, project_op->project_type_, new_input, non_common_projects, &non_common_project_op));
...
Status CommonColumnOptimize::ProcessTrivial(PhysicalPlanContext* ctx, PhysicalOpNode* op, BuildOpState* state) {
    bool changed = false;
    bool is_common = true;
...
static Status ReplaceRequestUnion(PhysicalPlanContext* ctx, PhysicalOpNode* root, PhysicalRequestUnionNode* target,
                                  PhysicalOpNode* repl, std::map<size_t, PhysicalOpNode*>* visited,
                                  PhysicalOpNode** output) {
    // check visisted
    ...
    if (root->GetOpType() == PhysicalOpType::kPhysicalOpRequestUnion) {
        ...
        CHECK_TRUE(request_union_op == target, kPlanError, "Found another request union node under window agg");
        *output = repl;
    ...
    for (size_t i = 0; i < root->GetProducerCnt(); ++i) {
        CHECK_STATUS(ReplaceRequestUnion(ctx, root->GetProducer(i), target, repl, visited, &new_children[i]));
    ...
    if (changed) {
        ...
        if (new_root->GetOpType() == PhysicalOpType::kPhysicalOpJoin && !new_children.empty() &&
            new_children[0]->GetOutputType() == PhysicalSchemaType::kSchemaTypeRow) {
            auto join_op = dynamic_cast<PhysicalJoinNode*>(new_root);
            PhysicalRequestJoinNode* request_join_op = nullptr;
            CHECK_STATUS(ctx->CreateOp<PhysicalRequestJoinNode>(&request_join_op, new_children[0], new_children[1],
...
Status CommonColumnOptimize::ProcessRequestUnion(PhysicalPlanContext* ctx, PhysicalRequestUnionNode* request_union_op,
    ...
    CHECK_STATUS(GetReorderedOp(ctx, request_union_op->GetProducer(1), &new_right));
    ...
    CHECK_STATUS(ctx->CreateOp<PhysicalRequestUnionNode>(
        &new_request_union, request_state->common_op, new_right, request_union_op->window(),
        request_union_op->instance_not_in_window(), request_union_op->exclude_current_time(), false));
    ...
    for (auto& pair : request_union_op->window_unions().window_unions_) {
        ...
        // reset window spec
        auto window_ptr = &new_request_union->window_unions_.window_unions_.back().second;
        CHECK_STATUS(ReplaceComponentExpr(pair.second, origin_input_op->schemas_ctx(),
                                          request_state->common_op->schemas_ctx(), ctx->node_manager(), window_ptr));
    }

    // apply ops path to window part
    std::map<size_t, PhysicalOpNode*> replace_cache;
    PhysicalOpNode* window_root = nullptr;
    CHECK_STATUS(ReplaceRequestUnion(ctx, path[0], request_union_op, new_request_union, &replace_cache, &window_root));
    ...
    CHECK_STATUS(ReplaceRequestUnion(ctx, path[0], request_union_op, origin_input_op, &replace_cache, &request_root));
    ...
    // final union
    Range request_ts;
    CHECK_STATUS(ReplaceComponentExpr(request_union_op->window().range_, origin_input_op->schemas_ctx(),
                                      request_state->common_op->schemas_ctx(), ctx->node_manager(), &request_ts));
    PhysicalPostRequestUnionNode* union_op = nullptr;
    CHECK_STATUS(ctx->CreateOp<PhysicalPostRequestUnionNode>(&union_op, optimized_request_root, optimized_window_root,
                                                             request_ts));
    *out = union_op;
    return Status::OK();
}

Status CommonColumnOptimize::ProcessWindow(PhysicalPlanContext* ctx, PhysicalAggrerationNode* agg_op,
                                           BuildOpState* state) {
    ...
    auto request_union_op = dynamic_cast<PhysicalRequestUnionNode*>(request_union_path.back());
    BuildOpState* request_state = nullptr;
    CHECK_STATUS(GetOpState(ctx, request_union_op->GetProducer(0), &request_state));

    // process request union path
    bool union_is_non_trivial_common = request_state->common_op != nullptr &&      // input has common part
                                       request_state->non_common_op != nullptr &&  // input is non-trivial
                                       LeftDependOnlyOnPart(request_union_op->window(), request_state->common_op,
    ...
    for (auto& pair : request_union_op->window_unions_.window_unions_) {
        union_is_non_trivial_common &=
            LeftDependOnlyOnPart(pair.second, request_state->common_op, request_union_op->GetProducer(0), nullptr);
    }

    PhysicalOpNode* new_union = nullptr;
    BuildOpState* agg_request_state = nullptr;
    if (union_is_non_trivial_common) {
        CHECK_STATUS(ProcessRequestUnion(ctx, request_union_op, request_union_path, &new_union, &agg_request_state));
    } else {
        CHECK_STATUS(GetOpState(ctx, input, &agg_request_state));
        CHECK_STATUS(GetReorderedOp(ctx, input, &new_union));
    }
    bool is_trival_common = !union_is_non_trivial_common && request_state->non_common_op == nullptr;
    ...
    std::unordered_map<size_t, size_t> new_id_map;
    for (size_t i = 0; i < agg_request_concat->GetOutputSchemaSourceSize(); ++i) {
    ...
    for (size_t i = 0; i < origin_projects.size(); ++i) {
        ...
        bool need_update = !ExprDependOnlyOnLeft(expr, agg_request_concat, nullptr);
        if (need_update) {
            node::ExprNode* new_expr = nullptr;
            CHECK_STATUS(UpdateProjectExpr(ctx, expr, input, new_id_map, &new_expr), "Fail to resolve expr ",
                         expr->GetExprString(), " on project input:\n", new_union->SchemaToString());
            expr = new_expr;
        }
        bool expr_is_common = is_trival_common || agg_request_state->non_common_op == nullptr ||
                              (union_is_non_trivial_common && ExprDependOnlyOnLeft(expr, agg_request_state->common_op,
    ...
    // create two new project ops
    if (common_projects.size() == origin_projects.size() && new_union == agg_op->GetProducer(0)) {
        state->common_op = agg_op;
    } else if (common_projects.size() > 0) {
        PhysicalAggrerationNode* common_project_op = nullptr;
        CHECK_STATUS(ctx->CreateOp<PhysicalAggrerationNode>(&common_project_op, new_union, common_projects));
        state->common_op = common_project_op;
    } else {
        state->common_op = nullptr;
    }

    if (non_common_projects.size() == origin_projects.size() && new_union == agg_op->GetProducer(0)) {
        state->non_common_op = agg_op;
    } else if (non_common_projects.size() > 0) {
        PhysicalAggrerationNode* non_common_project_op = nullptr;
        CHECK_STATUS(ctx->CreateOp<PhysicalAggrerationNode>(&non_common_project_op, new_union, non_common_projects));
        state->non_common_op = non_common_project_op;
    } else {
        state->non_common_op = nullptr;
    }
...
Status CommonColumnOptimize::ProcessConcat(PhysicalPlanContext* ctx, PhysicalRequestJoinNode* input,
                                           BuildOpState* state) {
    ...
    if (left_state->common_op == nullptr) {
        ...
        CHECK_STATUS(ctx->CreateOp<PhysicalRequestJoinNode>(&concat_op, left_state->common_op, right_state->common_op,
    ...
    if (left_state->non_common_op == nullptr) {
        ...
        PhysicalRequestJoinNode* concat_op = nullptr;
        CHECK_STATUS(ctx->CreateOp<PhysicalRequestJoinNode>(&concat_op, left_state->non_common_op,
                                                            right_state->non_common_op, node::kJoinTypeConcat));
    ...
    for (size_t right_idx : right_state->common_column_indices) {
        state->AddCommonIdx(right_idx + input->GetProducer(0)->GetOutputSchemaSize());
    }
...
Status CommonColumnOptimize::ProcessRename(PhysicalPlanContext* ctx, PhysicalRenameNode* input, BuildOpState* state) {
    BuildOpState* input_state = nullptr;
    ...
    if (input_state->common_op != nullptr && input_state->non_common_op != nullptr) {
        PhysicalRenameNode* common_rename_op = nullptr;
        CHECK_STATUS(ctx->CreateOp<PhysicalRenameNode>(&common_rename_op, input_state->common_op, alias_name));
        state->common_op = common_rename_op;

        PhysicalRenameNode* non_common_rename_op = nullptr;
        CHECK_STATUS(ctx->CreateOp<PhysicalRenameNode>(&non_common_rename_op, input_state->non_common_op, alias_name));
...
Status CommonColumnOptimize::ProcessJoin(PhysicalPlanContext* ctx, PhysicalRequestJoinNode* join_op,
                                         BuildOpState* state) {
    // process concat
    ...
    bool is_common_join =
        LeftDependOnlyOnPart(join_op->join(), left_state->common_op, join_op->GetProducer(0), join_op->GetProducer(1));
    bool is_non_common_join = LeftDependOnlyOnPart(join_op->join(), left_state->non_common_op, join_op->GetProducer(0),
                                                   join_op->GetProducer(1));
    ...
    if (is_common_join) {
        ...
        for (size_t i = 0; i < right->GetOutputSchemaSize(); ++i) {
            state->AddCommonIdx(join_op->GetProducer(0)->GetOutputSchemaSize() + i);
        }

        if (left_state->common_op == join_op->GetProducer(0) && right == join_op->GetProducer(1)) {
            state->common_op = join_op;
            state->non_common_op = nullptr;
        } else {
            PhysicalRequestJoinNode* new_join = nullptr;
            CHECK_STATUS(ctx->CreateOp<PhysicalRequestJoinNode>(&new_join, left_state->common_op, right,
                                                                join_op->join(), join_op->output_right_only()));
            ...
            state->non_common_op = join_op->output_right_only() ? nullptr : left_state->non_common_op;
        }
        ...
        PhysicalRequestJoinNode* new_join = nullptr;
        CHECK_STATUS(ctx->CreateOp<PhysicalRequestJoinNode>(&new_join, concat_left, right, join_op->join(), true));
        CHECK_STATUS(ReplaceComponentExpr(join_op->join(), join_op->joined_schemas_ctx(),
                                          new_join->joined_schemas_ctx(), ctx->node_manager(), &new_join->join_));
        ...
        if (join_op->output_right_only()) {
            ...
            CHECK_STATUS(ctx->CreateOp<PhysicalRequestJoinNode>(&concat_non_common, left_state->non_common_op, new_join,
                                                                node::kJoinTypeConcat));
            state->common_op = left_state->common_op;
...
Status CommonColumnOptimize::GetOpState(PhysicalPlanContext* ctx, PhysicalOpNode* input, BuildOpState** state_ptr) {
    // lookup cache
...
Status CommonColumnOptimize::GetConcatOp(PhysicalPlanContext* ctx, PhysicalOpNode* input, PhysicalOpNode** out) {
    BuildOpState* state = nullptr;
    CHECK_STATUS(GetOpState(ctx, input, &state));
    ...
    if (state->common_op == nullptr) {
        ...
        CHECK_STATUS(ctx->CreateOp<PhysicalRequestJoinNode>(&join, state->common_op, state->non_common_op,
                                                            node::kJoinTypeConcat));
        concat_op = join;
    }
...
Status CommonColumnOptimize::GetReorderedOp(PhysicalPlanContext* ctx, PhysicalOpNode* input, PhysicalOpNode** out) {
    BuildOpState* state = nullptr;
    CHECK_STATUS(GetOpState(ctx, input, &state));
    ...
    size_t common_size = state->common_op != nullptr ? state->common_op->GetOutputSchemaSize() : 0;
    ...
    if (input->GetOutputSchemaSourceSize() != concat_op->GetOutputSchemaSourceSize()) {
        need_reorder = true;
    } else {
        for (size_t i = 0; i < input->GetOutputSchemaSourceSize(); ++i) {
            if (input->GetOutputSchemaSource(i)->size() != concat_op->GetOutputSchemaSource(i)->size()) {
                need_reorder = true;
                break;
            }
        }
    ...
    for (size_t i = 0; i < state->common_column_indices.size(); ++i) {
        if (state->common_column_indices.find(i) == state->common_column_indices.end()) {
            need_reorder = true;
            break;
        }
    }
    ...
    for (size_t i = 0; i < input->GetOutputSchemaSize(); ++i) {
        ...
        if (iter != state->common_column_indices.end()) {
            ...
            select_column_id = get_column_id(concat_op, common_size + cur_non_common_idx);
        ...
        if (cur_projects.size() == cur_source->size()) {
            ...
            CHECK_STATUS(ctx->CreateOp<PhysicalSimpleProjectNode>(&select_op, concat_op, cur_projects));
            if (reordered_op == nullptr) {
                reordered_op = select_op;
            } else {
                PhysicalRequestJoinNode* next_concat = nullptr;
                CHECK_STATUS(ctx->CreateOp<PhysicalRequestJoinNode>(&next_concat, reordered_op, select_op,
                                                                    node::kJoinTypeConcat));
    ...
    CHECK_TRUE(reordered_op != nullptr && reordered_op->GetOutputSchemaSize() == input->GetOutputSchemaSize(),
               kPlanError);
