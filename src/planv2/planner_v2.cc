...
const std::string PlannerV2::GenerateName(const std::string prefix, int id) {
    time_t t;
    time(&t);
    std::string name = prefix + "_" + std::to_string(id) + "_" + std::to_string(t);
    return name;
}
bool PlannerV2::IsTable(node::PlanNode *node) {
    if (nullptr == node) {
        return false;
    }
...
bool PlannerV2::ValidatePrimaryPath(node::PlanNode *node, node::PlanNode **output, base::Status &status) {
    if (nullptr == node) {
    ...
    switch (node->type_) {
        ...
        case node::kPlanTypeUnion: {
            ...
            if (!ValidatePrimaryPath(binary_op->GetLeft(), &left_primary_table, status)) {
                status.msg = "primary path validate fail: left path isn't valid";
                status.code = common::kPlanError;
                LOG(WARNING) << status;
                return false;
            }
            ...
            if (!ValidatePrimaryPath(binary_op->GetRight(), &right_primary_table, status)) {
                status.msg = "primary path validate fail: right path isn't valid";
                status.code = common::kPlanError;
                LOG(WARNING) << status;
                return false;
            }
        ...
        case node::kProjectNode: {
            status.msg = "primary path validate fail: invalid node of primary path";
            status.code = common::kPlanError;
            LOG(WARNING) << status;
            return false;
        }
...
bool PlannerV2::CreateWindowPlanNode(const node::WindowDefNode *w_ptr, node::WindowPlanNode *w_node_ptr,
    ...
    if (nullptr != w_ptr) {
        ...
        if (w_ptr->GetName().empty()) {
            w_node_ptr->SetName(PlannerV2::GenerateName("anonymous_w", w_node_ptr->GetId()));
        } else {
...
std::string PlannerV2::MakeTableName(const PlanNode *node) const {
    switch (node->GetType()) {
        case node::kPlanTypeTable: {
...
bool PlannerV2::MergeProjectMap(const std::map<const node::WindowDefNode *, node::ProjectListNode *> &map,
...
bool PlannerV2::ExpandCurrentHistoryWindow(std::vector<const node::WindowDefNode *> *windows_ptr) {
    if (nullptr == windows_ptr) {
        return false;
    }
    auto &windows = *windows_ptr;
...
bool PlannerV2::CheckWindowFrame(const node::WindowDefNode *w_ptr, base::Status &status) {
    ...
    if (w_ptr->GetFrame()->frame_type() == node::kFrameRows) {
        ...
        if (w_ptr->GetFrame()->frame_maxsize() > 0) {
            status.code = common::kPlanError;
            status.msg = "Fail Make Rows Window: MAXSIZE non-support for Rows Window";
            LOG(WARNING) << status;
            return false;
        }
    }
...
bool PlannerV2::TransformTableDef(const std::string &table_name, const NodePointVector &column_desc_list,
                                  type::TableDef *table,
                                  Status &status) {  // NOLINT (runtime/references)
    ...
    for (auto column_desc : column_desc_list) {
        switch (column_desc->GetType()) {
            case node::kColumnDesc: {
                ...
                if (column_names.find(column_def->GetColumnName()) != column_names.end()) {
                ...
                switch (column_def->GetColumnType()) {
                    ...
                    case node::kVarchar:
                        ...
                        break;
                    default: {
                        status.msg = "CREATE common: column type " + node::DataTypeName(column_def->GetColumnType()) +
                                     " is not supported";
                        status.code = common::kSqlError;
                        LOG(WARNING) << status;
                        return false;
            ...
            case node::kColumnIndex: {
                ...
                if (index_names.find(column_index->GetName()) != index_names.end()) {
                    status.msg = "CREATE common: INDEX NAME " + column_index->GetName() + " duplicate";
                    status.code = common::kSqlError;
                    LOG(WARNING) << status;
                    return false;
                }
...
int SimplePlannerV2::CreatePlanTree(const zetasql::ASTScript *script, PlanNodeList &plan_trees,
                                    Status &status) {  // NOLINT (runtime/references)
    if (nullptr == script) {
