...
struct CountCateDef {
    ...
    struct Impl {
        ...
        static void Output(ContainerT* ptr, codec::StringRef* output) {
            ContainerT::OutputString(ptr, false, output, [](const int64_t& count, char* buf, size_t size) {
    ...
    struct Impl {
        ...
        static ContainerT* Update(ContainerT* ptr, InputV value, bool is_value_null, bool cond, bool is_cond_null,
            ...
            if (cond && !is_cond_null) {
                CountCateImpl::Update(ptr, value, is_value_null, key, is_key_null);
            }
            return ptr;
        }
    };
...
struct TopKCountCateWhereDef {
    ...
    struct Impl {
        ...
        static void Output(ContainerT* ptr, codec::StringRef* output) {
            ContainerT::OutputString(ptr, true, output, [](const int64_t& count, char* buf, size_t size) {
                return v1::format_string(count, buf, size);
            });
            ContainerT::Destroy(ptr);
        }
