...
#define DEFINE_REQUEST_WINDOW_CASE(NAME, PATH, CASE_ID)                                                                \
    static void BM_Request_##NAME(benchmark::State& state) {                                                           \
        auto sql_case =                                                                                                \
            hybridse::sqlcase::SqlCase::LoadSqlCaseWithID(hybridse::sqlcase::FindSqlCaseBaseDirPath(), PATH, CASE_ID); \
    ...
    BENCHMARK(BM_Request_##NAME)->ArgNames({"window_scale"})->Args({100})->Args({1000})->Args({2000});
...
DEFINE_REQUEST_WINDOW_CASE(BM_MultipleUDAF, "/cases/benchmark/udaf_benchmark.yaml", "0");
