...
TabletTableHandler::TabletTableHandler(const vm::Schema schema, const std::string& name, const std::string& db,
                                       const vm::IndexList& index_list, std::shared_ptr<storage::Table> table)
    : schema_(schema), name_(name), db_(db), table_(table), types_(), index_list_(index_list), tablet_() {}

TabletTableHandler::TabletTableHandler(const vm::Schema schema, const std::string& name, const std::string& db,
                                       const vm::IndexList& index_list, std::shared_ptr<storage::Table> table,
                                       std::shared_ptr<vm::Tablet> tablet)
    : schema_(schema), name_(name), db_(db), table_(table), types_(), index_list_(index_list), tablet_(tablet) {}
...
std::unique_ptr<RowIterator> TabletTableHandler::GetIterator() {
    std::unique_ptr<storage::FullTableIterator> it(
        new storage::FullTableIterator(table_->GetSegments(), table_->GetSegCnt(), table_));
    return std::move(it);
}

std::unique_ptr<WindowIterator> TabletTableHandler::GetWindowIterator(const std::string& idx_name) {
    ...
    std::unique_ptr<storage::WindowTableIterator> it(
        new storage::WindowTableIterator(table_->GetSegments(), table_->GetSegCnt(), iter->second.index, table_));
    return std::move(it);
}
...
RowIterator* TabletTableHandler::GetRawIterator() {
    return new storage::FullTableIterator(table_->GetSegments(), table_->GetSegCnt(), table_);
}
...
std::shared_ptr<vm::TableHandler> TabletCatalog::GetTable(const std::string& db, const std::string& table_name) {
...
bool TabletCatalog::AddTable(std::shared_ptr<TabletTableHandler> table) {
    ...
    if (db_it == tables_.end()) {
        tables_.insert(
            std::make_pair(table->GetDatabase(), std::map<std::string, std::shared_ptr<TabletTableHandler>>()));
...
TabletSegmentHandler::TabletSegmentHandler(std::shared_ptr<vm::PartitionHandler> partition_hander,
...
std::unique_ptr<WindowIterator> TabletSegmentHandler::GetWindowIterator(const std::string& idx_name) {
